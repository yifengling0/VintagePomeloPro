...
> ⚠️ **新会话请先读 `docs/HANDOVER_UI_2026-09-23-SESSION.md`**（最新入口，含已验证的铁律/链路/待办/工作区污染警告）。
> 本文保留 2026-09-23 之前的完整历史细节（§1–§15），其中**"自绘返回键会被 HDS 内部图层遮挡"这一旧结论已被推翻**（真实原因是标题栏未隐藏，改用 `.hideTitleBar(true)` 后自绘控件正常显示），以新文档 §4 为准。
>
...
## 13. 2026-09-23 增补三：环形封面流（提交 c1e854e）

### 13.1 形态（用户定案）
卡片排成**环**（弧线环绕），不是平铺直线、也不是"无限循环的直线"：
- **中心卡恒为焦点**：正对观众（rotationY=0）、最大（1.08）、最亮、accent 描边
- **两侧卡片向内旋转**（每格 26°）+ 下沉（每格 12vp）+ 缩小压暗 → 环绕纵深
- **一屏 5 格**（屏宽/5），左右自然流出留白
- **首尾无缝循环**：数据三倍化 + 边界无感跳转（`recycleRingIfNeeded`）
- **松手吸附**到最近格（`snapRingToNearest`），焦点位恒定
- 少于 5 张自动关闭循环

### 13.2 交互
点非中心卡 → 它转到中心（`selectRingIndex`）；点中心卡 → 待启动态（"再点启动"提示）→ 再点启动。滑动切换中心项时清空待启动态。

### 13.3 关键实现（三个必踩的坑）
1. **3D 变换必须在 AppCard 内部按 `ringOffset` 计算**，不能让页面算好传入 —— **ForEach 在 key 不变时不重新执行 itemGenerator**，外部传入的变换值不会随滚动更新（这是"滑动时画面完全不动"的真因之一）。
2. **横向滚动用 `List` 且 scroller 必须经构造参数绑定**：`List({ space: 0, scroller: this.ringScroller })`。漏绑却在 `onDidScroll` 里调 `this.ringScroller.currentOffset().xOffset` 会**直接闪退**（实测）。
3. `rotate` 需要完整参数：`.rotate({ x: 0, y: 1, z: 0, angle: deg })`，只给 `{y}` 编译不过。

### 13.4 ⚠️ 验证方法的重要教训（本次最大坑）
