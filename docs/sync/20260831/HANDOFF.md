...
当前双 ABI 候选包的运行时代码检查点是 `c5e00c9a`（含 beb 根会话、edd IME、dc 后台消费及明确退出码保留/初始化失败检查）；后续文档或主机测试工具提交不会自动改变已安装 HAP 的二进制来源。最终状态必须同时记录源码 SHA、HAP SHA-256、安装日志、设备结果，不能只记录版本号 1.3.3。最新状态/未通过项以 STATUS、NEXT_TASK 和 DEVICE_RESULTS 20 为准，不用旧检查点的测试替代新运行时验收。
