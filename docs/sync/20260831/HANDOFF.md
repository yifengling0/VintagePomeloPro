...
当前双 ABI 候选包的运行时代码检查点是 `dc287c9f`（含 beb 根会话、edd IME 生命周期和后台零拷贝消费修复）；后续文档或主机测试工具提交不会自动改变已安装 HAP 的二进制来源。最终状态必须同时记录源码 SHA、HAP SHA-256、安装日志、设备结果，不能只记录版本号 1.3.3。最新状态/未通过项以 STATUS、NEXT_TASK 和 DEVICE_RESULTS 19 为准，不用旧检查点的测试替代新运行时验收。
