...
2. **启动失败（I4）**：Terra 先只读 `startup-i4-minimal.json`（约 7 KB）和 OPEN_ISSUES I4。已定位手机入口会吞掉返回状态，未定位启动根因；新样本 root 存在、notepad 未出窗口，旧 B3 是零 toplevel。需要时才按 PID/时间读取原日志。后续单独诊断 loader/guest 退出与有限模块/SEH 事件；不凭统一 code=0 或两版共有的缺库告警判断成功/根因。不 reset prefix、无限重试或缩减门禁。
