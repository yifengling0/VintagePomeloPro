...
当前代码：`codex/sync-master-20260831`，运行时提交 `beb00711`；后续文档提交不改变二进制。原产品 main、其他工作树、所有子模块 pins 均未移动；没有 push。T1–T7 已完成，**不要重新移植 69 项或重搬 181 个 Native 文件**。
...
2. **冷启动失败（I4）**：Terra 先只读 `candidate-b3-failed.json`、该次 startup/stderr 摘要和成功 retry1 的相邻事件；定位 Explorer/loader/root 创建链。分清性能工具的 180 秒 readiness 与产品十分钟 watchdog；一次失败与重试成功均保留。不得直接 reset prefix、无限重试或缩减门禁。
3. **GL 告警（I1）**：Terra 只读一次 blit-drop 的 Native presenter/退避调用链及对应日志，区分前台、切后台、恢复后的时间段。两版都出现错误不代表频率相同。三个交替样本已完成，禁止为选出“漂亮结果”重新筛样本。
4. **已有兼容缺陷（I2/I5/I7）**：分别是引擎重建后中文、War3 共存边缘/首触摸、Legacy D3D9 初始化失败。原版对照已完成。每次仅处理一项；需要 guest/子模块变化时另开干净来源的独立功能任务，当前 pins 不动。不得把 UI 改版、绕过错误检查或空循环 FPS 当修复。
...
- 当前最新 HAP 为 `VintagePomeloPro-sync-beb00711-dual.hap`，SHA-256 `bb6e65b9718d9819e543505d1ddbb8504c38ba34b9cda7f3e21fc09956186fec`。实际产物、截图与日志在忽略目录 `.hvigor/outputs/sync-master-20260831/`；来源身份以安装记录和哈希为准。
- 121 个 Native 库及 wine-data.zip 与已验证 d28 包字节相同。Hvigor 曾移除根 build 中间目录；若要复用依赖，必须重新校验实际输入，不盲目假设缓存仍在，也不修改其他容器来修缓存。
